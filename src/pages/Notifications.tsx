...
const Notifications = () => {
  ...
  const fetchReadNotifications = async (uid: string) => {
    try {
      ...
      if (data && data.length > 0) {
        ...
        setReadNotificationIds(readNotifs.map(item => item.notification_id));
      }
    } catch (error) {
    ...
    try {
      ...
      const notificationsData: NotificationData[] = keyActivities.map((activity, index) => {
        ...
        return {
          ...
          type,
          read: readNotificationIds.includes(activity.id),
  ...
  const saveReadStatus = async (notificationId: string) => {
    ...
    try {
      // Insert into read_notifications table using a raw query
  ...
  const handleMarkAllAsRead = async () => {
    ...
    try {
      ...
      // Create batch of upsert objects
